...
export interface FulfillItemResponse {
  purchase_offer_id: number;
  offer_id: number;
  status: string;
  fulfilled_quantity: number;
...
export interface FulfillResponse {
  fulfilled_items: FulfillItemResponse[];
...
// Тип для упрощенного ответа списка покупок
...
export async function getPurchaseById(purchaseId: number): Promise<Purchase> {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(`Ошибка получения заказа: ${response.status} ${errorText}`);
  }
 ...
 */
export async function getPurchasesList(): Promise<PurchaseListItem[]> {
  try {
    const response = await authFetch(getApiUrl(API_ENDPOINTS.PURCHASES.LIST), {
      method: 'GET',
      requireAuth: true,
    });
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Ошибка получения списка покупок: ${response.status} ${errorText}`);
    }
    ...
    if (!Array.isArray(purchasesData)) {
      console.error('Неверный формат ответа от /purchases:', responseData);
      return [];
    }
...
/**
 * Проверяет токен и получает данные заказа для выдачи
 * @param token - Токен из QR кода
 * @returns Promise с данными заказа для выдачи
 */
export async function verifyToken(token: string): Promise<VerifyTokenResponse> {
  try {
    const response = await authFetch(getApiUrl(API_ENDPOINTS.PURCHASES.VERIFY_TOKEN), {
      method: 'POST',
      requireAuth: true,
      headers: {
        'Content-Type': 'application/json',
...
/**
 * Выполняет выдачу заказа
 * @param purchaseId - ID заказа
 * @param fulfillData - Данные о выдаче товаров
 * @returns Promise с результатом выдачи
 */
export async function fulfillPurchase(
  purchaseId: number,
  fulfillData: FulfillRequest
